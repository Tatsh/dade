...
def test_run_extract_only(make_amp_ark: _Builder, tmp_path: Path) -> None:
    archive = tmp_path / 'MAIN.ARK'
    archive.write_bytes(make_amp_ark((('gen/a.txt', b'AAA'),)))
    steps = pipeline.run(archive, tmp_path / 'out', convert=False)
    assert list(steps) == ['extract']
    assert steps['extract'].startswith('1 files')
    assert (tmp_path / 'out' / 'gen' / 'a.txt').is_file()


def test_run_converts_assets(make_amp_ark: _Builder, make_hmx_bitmap: _Builder, make_dtb: _Builder,
    ...
    out = tmp_path / 'out'
    steps = pipeline.run(archive, out)
...
def test_run_converts_disc_audio(make_amp_ark: _Builder, tmp_path: Path) -> None:
    archive = tmp_path / 'MAIN.ARK'
    archive.write_bytes(make_amp_ark((('gen/a.txt', b'AAA'),)))
    disc = tmp_path / 'AUDIO'
    disc.mkdir()
    (disc / 'SONG.STR').write_bytes(bytes(4096))
    steps = pipeline.run(archive, tmp_path / 'out', disc_audio=disc)
    assert steps['disc_audio'] == '1 songs'
    assert (tmp_path / 'out' / 'disc_audio' / 'SONG.wav').read_bytes()[:4] == b'RIFF'


def test_run_game(make_amp_ark: _Builder, make_hmx_bitmap: _Builder, make_dtb: _Builder,
                  make_milo: _Builder, make_v14_mesh: _Builder, make_samp_bank: _Builder,
                  make_vag: _Builder, tmp_path: Path) -> None:
    ...
    out = tmp_path / 'out'
    summary = pipeline.run_game(game_dir, out, jobs=1)
