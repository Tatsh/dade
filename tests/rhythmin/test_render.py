"""Tests for :py:mod:`dade.rhythmin.render`."""
...
def test_japanese_font_path_returns_none_without_fc_match(clear_font_caches: None,
    ...
    mocker.patch('dade.rhythmin.render.which', return_value=None)
    assert japanese_font_path() is None


def test_japanese_font_path_reports_the_matched_font(clear_font_caches: None,
                                                     mocker: MockerFixture) -> None:
    mocker.patch('dade.rhythmin.render.which', return_value='/usr/bin/fc-match')
    mocker.patch('dade.rhythmin.render.sp.run',
...
def test_japanese_font_path_returns_none_when_fc_match_cannot_run(clear_font_caches: None,
                                                                  mocker: MockerFixture) -> None:
    mocker.patch('dade.rhythmin.render.which', return_value='/usr/bin/fc-match')
    mocker.patch('dade.rhythmin.render.sp.run', side_effect=OSError)
    assert japanese_font_path() is None


def test_load_font_uses_the_built_in_font_without_a_match(clear_font_caches: None,
                                                          mocker: MockerFixture) -> None:
    mocker.patch('dade.rhythmin.render.japanese_font_path', return_value=None)
...
def test_load_font_loads_the_matched_font(clear_font_caches: None, mocker: MockerFixture) -> None:
    mocker.patch('dade.rhythmin.render.japanese_font_path', return_value='/fonts/x.otf')
    sentinel = object()
    truetype = mocker.patch('dade.rhythmin.render.ImageFont.truetype', return_value=sentinel)
...
def test_load_font_falls_back_when_the_matched_font_will_not_load(clear_font_caches: None,
                                                                  mocker: MockerFixture) -> None:
    mocker.patch('dade.rhythmin.render.japanese_font_path', return_value='/fonts/x.otf')
    mocker.patch('dade.rhythmin.render.ImageFont.truetype', side_effect=OSError)
    fallback = object()
    mocker.patch('dade.rhythmin.render.ImageFont.load_default', return_value=fallback)
