...
- :func:`destin.marmalade.derbh.unpack` - read a Derbh (``.dz``) archive.
- :func:`destin.marmalade.resgroup.parse` - parse an IwResGroup (``.group.bin``).
- :func:`~destin.marmalade.texture.decode_texture` / :func:`~destin.marmalade.font.decode_font` -
  decode ``CIwTexture`` / ``CIwGxFont`` to Pillow images.
- :func:`destin.marmalade.material.decode_material` - decode ``CIwMaterial``.
- :func:`destin.marmalade.model.decode_model` - decode ``CIwModel`` geometry.
- :func:`destin.marmalade.hashstring.iw_hash_string` - Marmalade ``IwHashString``.
"""
