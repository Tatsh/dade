...
__all__ = ('BLOWFISH_INIT_WORDS', 'DEFAULT_IV', 'BFCodec', 'Blowfish', 'decipher', 'encipher')
...
class Blowfish:
    ...
    init_words : Sequence[int]
        The initial P-array and S-boxes, defaulting to :py:data:`BLOWFISH_INIT_WORDS`.
...
class BFCodec:
    """
    The CBC wire format wrapped around :py:class:`Blowfish`.

    Parameters
    ----------
    key : bytes
        The cipher key, normally a 16-byte MD5 digest.
    iv : bytes
        The eight-byte initialisation vector, defaulting to :py:data:`DEFAULT_IV`.

    Raises
    ------
    ValueError
        If the initialisation vector is not eight bytes long, or the key is empty.
    """
...
def encipher(data: bytes, key: bytes) -> bytes:
    """
