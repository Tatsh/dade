...
status survive a rewrite untouched. That matters because the sequences are converted from the
game's own format and any re-quantising would change what is heard.
...
messages to another, which Extreme-G XG2 needs because it uses channel 10 as an ordinary bass part
that a General MIDI player would otherwise force to percussion.
"""
