...
Level sub-blobs and texture banks are LZHUF-compressed, which :py:mod:`destin.xg2.lzhuf` does not
...
def unpack(rom: bytes, out: Path, prefix: str = 'extreme-g') -> dict[str, int]:
    ...
    dict[str, int]
        ...
        :py:class:`~destin.xg2.mfs.MfsCalibrationError` propagates if the archive base cannot be
        calibrated.
    """
    out.mkdir(parents=True, exist_ok=True)
    boot = xg1_boot(rom)
