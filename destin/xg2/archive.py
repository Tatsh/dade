...
passing the appropriate :py:data:`~destin.xg2.typing.Endian` character, so the two platforms share
one parser.
"""
...
def parse_archive(data: bytes, base: int = 0, endian: Endian = '>') -> list[ArchEntry]:
    ...
    Returns
    -------
    list[destin.xg2.typing.ArchEntry]
...
def decode_entry(data: bytes, entry: ArchEntry) -> bytes:
    ...
    data : bytes
        Buffer holding the container.
    entry : destin.xg2.typing.ArchEntry
    ...
    bytes
        ...
        :py:class:`~destin.xg2.lzhuf.LzhufUnavailableError` propagates for an ``LHUF`` entry.
...
def decode_entries(data: bytes, entries: list[ArchEntry]) -> Iterator[tuple[ArchEntry, bytes]]:
    ...
    data : bytes
        Buffer holding the container.
    entries : list[destin.xg2.typing.ArchEntry]
        Records to decode, as returned by :py:func:`parse_archive`.

    Yields
    ------
    tuple[destin.xg2.typing.ArchEntry, bytes]
