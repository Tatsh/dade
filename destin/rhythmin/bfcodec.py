...
game: they are :data:`BLOWFISH_INIT_WORDS`, and with the standard F they reproduce every published
Blowfish known-answer vector.

The wire format ``BFCodec`` writes is CBC with a fixed initialisation vector (:data:`DEFAULT_IV`)
...
Every file the game ships or downloads uses the same key: the MD5 of :data:`KEY_PLAINTEXT`, which
is what :func:`default_key` returns. The key is spelled in the binary as ``key[i] + i``, so it is
obfuscated rather than hidden. Purchased music uses the MD5 of the device UUID instead, which is
why :class:`BFCodec` takes an arbitrary key.
"""
...
:meta hide-value:
"""
DEFAULT_IV = bytes((0xE3, 0x66, 0x31, 0xDA, 0x2C, 0x85, 0xA0, 0x64))
"""Fixed CBC initialisation vector the game seeds every chain with.
...
"""Canonical Blowfish initial P-array (18 words) followed by the four 256-word S-boxes.

These are the fractional hexadecimal digits of pi, the standard Blowfish constants.
...
def default_key() -> bytes:
    ...
    -------
    bytes
        The 16-byte MD5 of :data:`KEY_PLAINTEXT`.
    """
...
class Blowfish:
    """
    The game's Blowfish block cipher, with its non-standard F function.

    Parameters
    ----------
    key : bytes
        The key, of any non-zero length. It is cycled over the P-array exactly as standard Blowfish
        cycles it.
    init_words : Sequence[int]
        The initial P-array and S-boxes, defaulting to :data:`BLOWFISH_INIT_WORDS`.

    Raises
    ------
    ValueError
        If the key is empty or the initial table is not the expected length.
    """
    def __init__(self, key: bytes, init_words: Sequence[int] = BLOWFISH_INIT_WORDS) -> None:
        if not key:
            msg = 'The key must not be empty.'
        ...
        self._s = [
            list(init_words[_P_WORDS + box * _S_BOX_SIZE:_P_WORDS + (box + 1) * _S_BOX_SIZE])
            for box in range(_S_BOX_COUNT)
        ]
        position = 0
        for index in range(_P_WORDS):
            word = 0
            for _ in range(4):
                word = ((word << 8) | key[position % len(key)]) & _MASK32
                position += 1
            self._p[index] ^= word
        # Diffuse the schedule by repeatedly enciphering a running block through the P-array and
        # then every S-box, each pair of words replacing the block that produced it.
        left = right = 0
        for index in range(0, _P_WORDS, 2):
            left, right = self.encrypt_block(left, right)
            self._p[index], self._p[index + 1] = left, right
        for box in self._s:
            for index in range(0, _S_BOX_SIZE, 2):
                left, right = self.encrypt_block(left, right)
                box[index], box[index + 1] = left, right

    def encrypt_block(self, left: int, right: int) -> tuple[int, int]:
        """
        Encipher one block, given as its two 32-bit halves.

        Parameters
        ----------
        left : int
            The block's high half.
        right : int
            The block's low half.
        ...
        for index in range(0, _ROUNDS, 2):
            left ^= self._p[index]
            right = (right ^ self._f(left)) & _MASK32
            right ^= self._p[index + 1]
            left = (left ^ self._f(right)) & _MASK32
        left ^= self._p[_ROUNDS]
        right ^= self._p[_ROUNDS + 1]
        return right & _MASK32, left & _MASK32

    def decrypt_block(self, left: int, right: int) -> tuple[int, int]:
        """
        Decipher one block, given as its two 32-bit halves.
        ...
        -------
        tuple[int, int]
            The deciphered halves.
        """
        for index in range(_ROUNDS, 1, -2):
            left ^= self._p[index + 1]
            right = (right ^ self._f(left)) & _MASK32
            right ^= self._p[index]
            left = (left ^ self._f(right)) & _MASK32
        left ^= self._p[1]
    ...
    def _f(self, x: int) -> int:
        ...
        ----------
        x : int
            The 32-bit round input.

        Returns
        -------
        int
            ``(S0[a] + S1[b]) ^ (S2[c] + S3[d])``, where standard Blowfish would compute
            ``((S0[a] + S1[b]) ^ S2[c]) + S3[d]``.
        """
        return (((self._s[0][(x >> 24) & 0xFF] + self._s[1][(x >> 16) & 0xFF]) & _MASK32) ^
                ((self._s[2][(x >> 8) & 0xFF] + self._s[3][x & 0xFF]) & _MASK32)) & _MASK32


class BFCodec:
    """
    The CBC wire format wrapped around :class:`Blowfish`.

    Parameters
    ----------
    key : bytes | None
        The cipher key, defaulting to :func:`default_key`.
    iv : bytes
        The eight-byte initialisation vector, defaulting to :data:`DEFAULT_IV`.

    Raises
    ------
    ValueError
        If the initialisation vector is not eight bytes long.
    """
    def __init__(self, key: bytes | None = None, iv: bytes = DEFAULT_IV) -> None:
        if len(iv) != _BLOCK_SIZE:
            msg = f'The initialisation vector must be {_BLOCK_SIZE} bytes, not {len(iv)}.'
            raise ValueError(msg)
        self._cipher = Blowfish(default_key() if key is None else key)
        self._iv = bytes(iv)

    def encipher(self, data: bytes) -> bytes:
        """
        Encipher a payload into the game's wire format.
        ...
        -------
        bytes
            The ciphertext: the zero-padded body followed by the eight-byte length trailer.
        """
        padded = data.ljust((len(data) + 7) & ~7, b'\0')
        chain_left = int.from_bytes(self._iv[:4], 'big')
        chain_right = int.from_bytes(self._iv[4:], 'big')
        out = bytearray()
        for offset in range(0, len(padded), _BLOCK_SIZE):
            left = int.from_bytes(padded[offset:offset + 4], 'big') ^ chain_left
            right = int.from_bytes(padded[offset + 4:offset + _BLOCK_SIZE], 'big') ^ chain_right
            chain_left, chain_right = self._cipher.encrypt_block(left, right)
            out += chain_left.to_bytes(4, 'big') + chain_right.to_bytes(4, 'big')
        return bytes(out) + len(data).to_bytes(4, 'big') + len(padded).to_bytes(4, 'big')

    def decipher(self, data: bytes) -> bytes:
        """
        Decipher the game's wire format back to the original payload.
        ...
        -------
        bytes
            The plaintext, truncated to the length the trailer records.
        ...
        if len(data) < _TRAILER_SIZE:
            msg = f'Too short for the {_TRAILER_SIZE}-byte length trailer: {len(data)} bytes.'
            raise ValueError(msg)
        body = len(data) - _TRAILER_SIZE
        original_length = int.from_bytes(data[body:body + 4], 'big')
        padded_length = int.from_bytes(data[-4:], 'big')
        if padded_length != body or body != ((original_length + 7) & ~7):
            msg = (f'Bad length trailer: original {original_length}, padded {padded_length}, '
                   f'body {body}.')
            raise ValueError(msg)
        chain_left = int.from_bytes(self._iv[:4], 'big')
        chain_right = int.from_bytes(self._iv[4:], 'big')
        out = bytearray()
        for offset in range(0, body, _BLOCK_SIZE):
            cipher_left = int.from_bytes(data[offset:offset + 4], 'big')
            cipher_right = int.from_bytes(data[offset + 4:offset + _BLOCK_SIZE], 'big')
            left, right = self._cipher.decrypt_block(cipher_left, cipher_right)
            out += ((left ^ chain_left) & _MASK32).to_bytes(4, 'big')
            out += ((right ^ chain_right) & _MASK32).to_bytes(4, 'big')
