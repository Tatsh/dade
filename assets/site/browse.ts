// Finding a tune in the collection: what the index stores, how a search matches, and how the list
// is filed. None of it touches the page, and it can therefore be checked on its own.
import { type Chart } from './chart/types';

/** How a tune's charts are identified, easiest first. The last is harder than hard. */
 ...
 *
 * Written out rather than deferred to `text-transform`. `text-transform` capitalises by word and
 * therefore depends on where the spaces happen to fall.
 */
...
export interface Tune {
  ...
  letter: string;
  /** Each chart it includes, against the level the metadata gives it. */
  levels: Partial<Record<Difficulty, number | null>>;
  /** Which gojūon row it is filed under. */
  row: string;
  /** The identifier of the extend note with its SPECIAL chart, or null. */
  special: number | null;
...
export interface Index {
  ...
  letters: string[];
  /** Every gojūon row some tune is filed under, in the gojūon's order. */
  rows: string[];
  /** Every tune. */
  tunes: Tune[];
}
...
/**
 * The six fields a search is matched against.
 *
 * A tune can be looked for by what it is called, by how the title is read, or by how the reading is
...
/** Everything that is not a letter or a digit. A search should not have to type it. */
const NOISE = /[^\p{L}\p{N}]+/gu;

/**
 * The sounds that are written one way and typed another.
 *
 * The romanisation is Hepburn, the way a reading is *written*. `し` is `shi` and the particle `を`
 * is `o`. Hepburn is not how a reading is *typed*. A Japanese keyboard takes `si` and `wo`. Neither
 * spelling is wrong. Both sides of a search are therefore folded to one of them, and either
 * spelling finds the tune. The longer sound comes first, and `shi` is therefore folded before `sh`.
 */
...
/**
 * Whether a tune matches what was typed.
 *
 * Punctuation, case, and width are ignored on both sides. `kors k` therefore finds `KORS K`, and
 * `クシコス` finds `クシコス☆ポスト`. The difference between how a reading is written and how it is
 * typed is ignored as well, and `愛を` therefore matches both `aio` and `aiwo`.
 *
 * A search of several words requires all of them, each anywhere the tune can be searched. A title
 * and an artist can therefore be given together, and the words need not be in the order the tune
 * has them. Each word still has to be found whole in one field. A word is not made up out of the
 * end of one field and the start of the next. An empty search matches everything.
 */
...
/**
 * File the tunes under their headings, omitting any heading nothing is filed under.
 *
...
export const group = (tunes: Tune[], filing: Filing, headings: string[]): Group[] => {
  ...
  // A tune filed under a heading the index did not list is still shown, after the ones it did.
...
/** Which charts a tune includes, easiest first. */
export const difficulties = (tune: Tune) => DIFFICULTIES.filter((name) => name in tune.levels);

/** What one route identifies. A route with no tune is the list. */
 ...
 * page it was built for serves a `404.html` copy. A deep link therefore boots the app. Without one
 * it cannot know where its files are. It falls back to the hash, and the hash works from anywhere.
 */
...
/** Write a route as a value that can be given to the browser. */
