...
- :func:`dade.thps2pc.pkr.parse` and :func:`dade.thps2pc.pkr.extract_all` - read a ``PKR2``
  resource pack such as ``All.pkr``.
- :class:`dade.thps2pc.psx.Scene` - parse a ``.PSX`` scene: sectors, faces, mesh-section
  descriptors, and the chunk list.
- :func:`dade.thps2pc.textures.parse_lighting` and
  :func:`dade.thps2pc.textures.decode_instance` - decode the palettes and textures embedded in
  a ``*_L.PSX`` lighting file.
- :func:`dade.thps2pc.mesh.build_batches` - group a scene's triangles by texture for export.
