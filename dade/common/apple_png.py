...
handles the result. ``pngdefry`` undoes all three, and passes an ordinary PNG through unchanged.

Every iOS title handled here ships images in this form, and the conversion therefore lives here
rather than in
...
PNG_MAGIC = b'\x89PNG\r\n\x1a\n'
"""The PNG signature, also distinguishing a decipher that worked from one that did not.
...
def is_apple_optimized(data: bytes) -> bool:
    """
    Report whether a PNG includes the ``CgBI`` chunk Xcode adds.

    The chunk is required to come first, before ``IHDR``, and only that position is examined.
...
def defry_png(source: Path, destination: Path, pngdefry: Path) -> bool:
    ...
    ``pngdefry`` passes an unoptimised PNG through, writing nothing and still succeeding,
    ...
    """
    # pngdefry titles its output after the input and can only be pointed at a directory. It runs
...
def write_defried_png(source: Path, destination: Path, pngdefry: Path) -> Path:
    ...
    -------
    pathlib.Path
        The written file, always *destination* whether it was converted or copied.
    """
    if not defry_png(source, destination, pngdefry):
        # Not an Apple-optimised PNG. It is already readable everywhere and is copied as it is.
