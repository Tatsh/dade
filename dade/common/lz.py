"""
Okumura LZSS decompressor, binary ``_0`` variant, as used by both Extreme-G games.

The ring buffer is 4096 bytes and zero-filled, the write cursor starts at ``0xFEE``, and control
bytes are consumed least-significant bit first. A set control bit introduces one literal byte; a
clear bit introduces a two-byte match where the offset is ``b0 | ((b1 & 0xF0) << 4)`` and the
length is ``(b1 & 0x0F) + 3``. The stream carries no end marker: decoding stops once the caller's
expected output size is reached.

The PC port stores the identical byte stream, so this decoder serves both platforms unchanged.
"""
...
def decompress_lzss0(data: bytes, start: int, decompressed_size: int) -> tuple[bytes, int]:
    """
